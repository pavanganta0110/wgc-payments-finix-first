...
type SortKey = "brand" | "received" | "authorized" | "voided" | "authorizedAmount" | "rateValue";

export default function CardAuthorizationDataTable({
  rows,
  dimensionLabel,
}: {
  ...
  const columns: { key: SortKey; label: string }[] = [
    ...
    { key: "received", label: "Received Count" },
    { key: "authorized", label: "Authorized Count" },
    { key: "voided", label: "Voided Count" },
    { key: "authorizedAmount", label: "Authorized Amount" },
    { key: "rateValue", label: "Authorization Rate" },
  ];
      ...
      <tbody>
        {sorted.map((row) => (
          <tr key={row.brand} className="border-t border-slate-50">
            ...
            <td className="px-6 py-3">{row.received}</td>
            <td className="px-6 py-3">{row.authorized}</td>
            <td className="px-6 py-3">{row.voided}</td>
            <td className="px-6 py-3">{row.authorizedAmount}</td>
