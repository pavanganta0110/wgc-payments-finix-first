...
type SortKey = "brand" | "volumeCents" | "count" | "failedVolumeCents" | "failedCount" | "successRatio";

export default function CardPaymentDataTable({
  rows,
  dimensionLabel,
}: {
  rows: CardPaymentRow[];
  ...
  const columns: { key: SortKey; label: string }[] = [
    ...
    { key: "volumeCents", label: "Transaction Volume" },
    { key: "count", label: "Transaction Count" },
    { key: "failedVolumeCents", label: "Failed Vol" },
    { key: "failedCount", label: "Failed Count" },
    { key: "successRatio", label: "Success Ratio" },
  ];
      ...
      <tbody>
        {sorted.map((row) => (
          <tr key={row.brand} className="border-t border-slate-50">
            ...
            <td className="px-6 py-3">{row.failedVolume}</td>
            <td className="px-6 py-3">{row.failedCount}</td>
            <td className="px-6 py-3">{row.successRatio}</td>
          </tr>
        ))}
      </tbody>
    </table>
