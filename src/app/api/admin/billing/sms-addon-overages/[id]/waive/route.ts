...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!reason) {
    return NextResponse.json({ error: "A reason is required to waive an overage." }, { status: 400 });
  }

  const { id } = await params;

  try {
    const result = await waiveSmsOverage(id, { userId: session.userId, email: session.email, role: session.role }, reason);
    return NextResponse.json({ success: true, overage: result });
  } catch (err) {
    if (err instanceof SmsOverageChargeError) {
      return NextResponse.json({ error: err.message }, { status: 400 });
    }
    console.error("SMS overage waive failed:", err);
