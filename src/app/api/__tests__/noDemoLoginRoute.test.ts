...
// CVE-class regression guard: src/app/api/demo/login/route.ts previously
// existed in this (production) repository and created a real, fully
// authenticated OWNER-role session for a real merchant
// (admin@gracecommunity.org / Grace Community Church) with zero
// authentication check — anyone who requested the URL was logged in.
// The fix was full removal from this repo (not an env-var gate) since a
// route that doesn't exist can never be re-enabled by a misconfigured
// environment variable. This test fails the build if the route file, or
// any other file matching the same "demo login" shape, is ever
// reintroduced here.
describe("no unauthenticated demo/test login route exists in this repo", () => {
